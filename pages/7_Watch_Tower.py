...
def get_active_countries():
    ...
    try:
        ...
        # Assuming the column is named 'Geographic Zone'
...
def main():
    ...
    with col1:
        # Later, this will be fetched dynamically from the Tenant Profile
        categories = st.multiselect(
            "Product Scope (Tenant Categories)",
            ["E-Bikes (EPAC)", "Smartwatches", "Audio Headsets", "Sportswear", "General Electronics"],
            default=["E-Bikes (EPAC)", "Smartwatches"]
        )
    with col2:
        # Dynamic country list from your CSV
    ...
    if run_scan:
        ...
        tab_inbox, tab_bookmark, tab_archive = st.tabs(["📥 Inbox (Unread)", "📌 Bookmarked", "🗄️ Archive (Audit Trail)"])

        with tab_inbox:
            st.markdown("### 🔴 High Priority (Action Required)")
            
            # Card 1: Draft Regulation (High Priority)
            with st.container(border=True):
                st.markdown("#### 📄 [Draft] EU Battery Regulation: New removability requirements for e-scooters")
                st.caption("🌍 **Market:** EU | 🏛️ **Source:** [eur-lex.europa.eu](#) | 📅 **Published:** 2026-07-14 | ⏱️ **Detected:** Today")
                ...
                with col_c:
                    st.button("📌 Pin", key="pin_1", use_container_width=True)
                with col_d:
                    st.button("🚫 Dismiss", key="dismiss_1", use_container_width=True)

            # Card 2: Guidance (High Priority - Immediate implementation)
            with st.container(border=True):
                st.markdown("#### 📘 [Guidance] ANSES: New restrictions on flame retardants")
                st.caption("🌍 **Market:** France | 🏛️ **Source:** [anses.fr](#) | 📅 **Published:** 2026-07-20 | ⏱️ **Detected:** Yesterday")
                ...
                with col_c:
                    st.button("📌 Pin", key="pin_2", use_container_width=True)
                with col_d:
                    st.button("🚫 Dismiss", key="dismiss_2", use_container_width=True)

            st.markdown("### 🟡 Medium Priority (Monitor)")
            
            # Card 3: In Force (Medium Priority - Minor update)
            with st.container(border=True):
                st.markdown("#### ⚖️ [In Force] MIIT: Standard for wireless wearable devices updated")
                st.caption("🌍 **Market:** China | 🏛️ **Source:** [miit.gov.cn](#) | 📅 **Published:** 2026-07-18 | ⏱️ **Detected:** 3 days ago")
                ...
                with col_c:
                    st.button("📌 Pin", key="pin_3", use_container_width=True)
                with col_d:
                    st.button("🚫 Dismiss", key="dismiss_3", use_container_width=True)

        with tab_bookmark:
            st.markdown("*Your pinned signals will appear here for later review.*")
